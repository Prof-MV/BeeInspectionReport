...
struct rc522 {
    ...
    i2c_master_dev_handle_t i2c_dev_handle; /*<! I2C device handle (new driver) */
...
static esp_err_t rc522_write_n(rc522_handle_t rc522, uint8_t addr, uint8_t n, uint8_t *data)
{
    ...
    // /* Mem check */
    // if (!buffer) {
    //     ESP_LOGE(TAG, "Failed to alloc memery");
...
rc522_config_t* rc522_clone_config(rc522_config_t* config)
{
    ...
    // clock_speed_hz removed - speed is fixed at RC522_DEFAULT_I2C_CLOCK_SPEED_HZ when adding the device
...
static esp_err_t rc522_create_transport(rc522_handle_t rc522)
{
    ...
    switch(rc522->config->transport) {
        case RC522_TRANSPORT_SPI: {
            ...
            break;
        case RC522_TRANSPORT_I2C:
            /* I2C device is added in rc522_create() using the new I2C master driver */
            break;
        default:
...
esp_err_t rc522_create(rc522_config_t* config, rc522_handle_t* out_rc522)
{
    ...
    if (config->transport == RC522_TRANSPORT_I2C) {
        i2c_device_config_t dev_config = {
            .dev_addr_length = I2C_ADDR_BIT_LEN_7,
            .device_address = RC522_I2C_ADDRESS,
            .scl_speed_hz = RC522_DEFAULT_I2C_CLOCK_SPEED_HZ,
        };
        if (ESP_OK != (ret = i2c_master_bus_add_device(config->i2c.bus_handle, &dev_config, &rc522->i2c_dev_handle))) {
...
static void rc522_destroy_transport(rc522_handle_t rc522)
{
    switch(rc522->config->transport) {
        case RC522_TRANSPORT_SPI:
            ...
            break;
        case RC522_TRANSPORT_I2C:
            if (rc522->i2c_dev_handle) {
                i2c_master_bus_rm_device(rc522->i2c_dev_handle);
                rc522->i2c_dev_handle = NULL;
            }
            break;
        default:
...
static esp_err_t rc522_i2c_send(rc522_handle_t rc522, uint8_t* buffer, uint8_t length)
{
    return i2c_master_transmit(rc522->i2c_dev_handle, buffer, length, rc522->config->i2c.rw_timeout_ms);
}

static esp_err_t rc522_i2c_receive(rc522_handle_t rc522, uint8_t* buffer, uint8_t length, uint8_t addr)
{
    return i2c_master_transmit_receive(rc522->i2c_dev_handle, &addr, 1, buffer, length, rc522->config->i2c.rw_timeout_ms);
}
