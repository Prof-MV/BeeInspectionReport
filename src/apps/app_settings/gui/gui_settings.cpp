...
void GUI_Settings::renderMainMenu(int selectedOption)
{
    ...
    _canvas->drawCenterString("SETTINGS", bubble.x, bubble.y - 60);

    // Menu options
    _canvas->setFont(GUI_FONT_CN_SMALL);

    const char* options[] = {"Timezone", "Clear Data", "Done"};
    int optionCount = 3;
    ...
    for (int i = 0; i < optionCount; i++) {
        ...
        if (i == selectedOption) {
            ...
            _canvas->fillRoundRect(bubble.x - 80, y - 12, 160, 28, 8, _theme_color);
    ...
    _canvas->setFont(GUI_FONT_CN_SMALL);
    _canvas->setTextColor(TFT_DARKGRAY);
    _canvas->drawCenterString("rotate to select", bubble.x, bubble.y + 55);
